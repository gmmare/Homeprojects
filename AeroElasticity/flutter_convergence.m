...
flut_list = [];
elem_list = linspace(5,50,46);
for i =1:length(elem_list)

%% Finite element settings
Ne = elem_list(i);            % Number of elements
...
if length(flut_point) > 1
    if flut_point(end)/flut_point(end-1) <0
        flut_list = [flut_list, V];
    end
end
...
%%
figure(1)
plot(elem_list, flut_list)
ylabel('Flutterspeed [ms^{-1}]')
